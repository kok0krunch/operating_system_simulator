...
try:
    from .fcfs_cpu import fcfs_menu
    from .sjf import sjf_menu
    from .priority import priority_menu
    from .round_robin import rr_menu
...
# Create function for CPU Scheduling Menu to be imported to main.py

def main_cpu_menu():
    pygame.init()
    screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))
    pygame.display.set_caption("CPU Scheduling Simulator")
    ...
    menu_options = [
        ("[1] First Come, First Served Scheduling", fcfs_menu),
        ("[2] Shortest Job First Scheduling", sjf_menu),
        ("[3] Priority Scheduling", priority_menu),
        ("[4] Round-Robin Scheduling", rr_menu)
    ]
    ...
    while running:
        ...
        title_surf = font_header.render("CPU SCHEDULING", True, NEON_GREEN)
