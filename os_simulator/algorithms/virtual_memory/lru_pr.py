...
def lru_menu(screen):
    """
    Main GUI function for LRU Page Replacement.
    ...
    def find_lru_index(frames, seq, current_idx, cap):
        least_recent_idx = current_idx
        index_to_replace = 0
        ...
        for frame_idx in range(cap):
            ...
            # Look backward from the current position down to the past index
            for past_index in range(current_idx - 1, -1, -1):
                if seq[past_index] == current_frame_page:
                    found_in_past = True
                    if past_index < least_recent_idx:
                        least_recent_idx = past_index
                        index_to_replace = frame_idx
                    break
            
            # If the page was never used in the past, it's natively the oldest/least recently used
            if not found_in_past:
                return frame_idx
                
        return index_to_replace

    def run_lru_simulation(seq, cap):
        ...
        for i, page in enumerate(seq):
            ...
            if page in frames:
                ...
                # Case 2: Memory is full. Run backward look search.
                else:
                    replaced_index = find_lru_index(frames, seq, i, cap)
                    frames[replaced_index] = page
    ...
    while running:
                    ...
                    if event.key == pygame.K_RETURN:
                        ...
                        try:
                            ...
                            if not page_sequence:
                                ...
                                error_message = ""
                                run_lru_simulation(page_sequence, capacity)
                                state = 2
                        except ValueError:
        ...
        # 1. Top Left Title Bar
        title_surface = font_title.render("VIRTUAL MEMORY: Least Recently Used (LRU) Page Replacement", True, BLACK)
        screen.blit(title_surface, (20, 10))

        # 2. Setup Screen Interactions (Perfectly centered layouts with size 48 text spacing)
        if state == 0:
            ...
            for idx, step in enumerate(simulation_steps):
                ...
                # Frames Block Rendering (Skips drawing column values during Hit events)
...
if __name__ == "__main__":
    ...
    pygame.display.set_caption("Virtual Memory LRU Page Replacement")
    lru_menu(main_screen)