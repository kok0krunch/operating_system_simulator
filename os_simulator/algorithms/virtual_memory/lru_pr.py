...
class LRUPageReplacement:
    ...
    def run_simulation(self, page_sequence):
        print("\n" + "="*50)
        print("SIMULATION TABLE OUTPUT (LRU)")
        print("="*50)
        ...
        for i, page in enumerate(page_sequence):
            ...
            # PAGE FAULT: If the page isn't in memory, we need to add it
                ...
                # Case 2: Memory is full. Run the LRU backward-looking search.
                else:
                    replace_index = self.find_lru_index(page_sequence, i)
    ...
    def find_lru_index(self, page_sequence, current_index):
        # Looks backward into the past of the page_sequence to find which currently loaded page frame was used longest ago.
        least_recent_index = current_index
        index_to_replace = 0
        
        for frame_idx in range(self.capacity):
            current_frame_page = self.frames[frame_idx]
            
            # Loop backwards from the current position down to 0
            for past_index in range(current_index - 1, -1, -1):
                if page_sequence[past_index] == current_frame_page:
                    if past_index < least_recent_index:
                        least_recent_index = past_index
                        index_to_replace = frame_idx
                    break
            else:
                # If we never found the page in the past, it means it's the least recently used
                return frame_idx
