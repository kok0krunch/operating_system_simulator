...
def optimal_menu(screen):
    """
    Main GUI function for Optimal Page Replacement.
    ...
    # Define font instances strictly matching your customized setup sizes
    ...
    def find_optimal_index(frames, seq, current_idx, cap):
        furthest_idx = -1
        index_to_replace = 0
        ...
        for frame_idx in range(cap):
            ...
            for future_idx in range(current_idx + 1, len(seq)):
                if seq[future_idx] == current_frame_page:
            ...
            # If a page is never used again in the upcoming lookahead, swap it instantly
            if not found_in_future:
                return frame_idx
                
        return index_to_replace

    def run_optimal_simulation(seq, cap):
        ...
        for i, page in enumerate(seq):
            ...
            if page in frames:
                ...
                # Case 1: Fill empty slots sequentially if available
                if None in frames:
                    replaced_index = frames.index(None)
                    frames[replaced_index] = page
                # Case 2: Run lookahead index replacement evaluation
                else:
                    replaced_index = find_optimal_index(frames, seq, i, cap)
                    frames[replaced_index] = page
    ...
    while running:
                    ...
                    if event.key == pygame.K_RETURN:
                        ...
                        try:
                            ...
                            if not page_sequence:
                                ...
                                error_message = ""
                                run_optimal_simulation(page_sequence, capacity)
                                state = 2
                        except ValueError:
        ...
        title_surface = font_title.render("VIRTUAL MEMORY: Optimal Page Replacement", True, BLACK)
        screen.blit(title_surface, (20, 10))

        # 2. Setup Screen Interactions with customized gaps
        if state == 0:
            ...
            # 3. Diagram Simulation Grid Map Layout (Matches FIFO spacing parameters)
            ...
            for idx, step in enumerate(simulation_steps):
                ...
                # Frames Block Rendering (Skips on Hit exactly like the specification)
                if step.get("status") == "FAULT":
                    ...
                    for f_idx in range(capacity):
                        ...
                        if f_idx < len(step_frames):
                            ...
                            if val != "":
                                # Coloring updates red if index parameter was modified
...
if __name__ == "__main__":
    ...
    pygame.display.set_caption("Virtual Memory Optimal Page Replacement")
    optimal_menu(main_screen)